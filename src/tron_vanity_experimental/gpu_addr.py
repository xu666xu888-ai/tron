...
try:
    from .gpu_secp256k1 import (
        gpu_secp256k1_batch as secp_gpu_batch,
    ...
    secp_gpu_batch_w6 = None
    secp_gpu_batch_w8 = None
    _warmup_window4_table = None
    _warmup_window6_table = None
    _warmup_window8_table = None
else:
    secp_gpu_batch_w4 = gpu_secp256k1_batch_window4
    secp_gpu_batch_w6 = gpu_secp256k1_batch_window6
    secp_gpu_batch_w8 = gpu_secp256k1_batch_window8
    _warmup_window4_table = warmup_window4_table
    _warmup_window6_table = warmup_window6_table
    _warmup_window8_table = warmup_window8_table
...
def _normalize_wnaf_window(value: Optional[str]) -> int:
    ...
    try:
        ...
        return 0
    return parsed if parsed in (4, 6, 8) else 0


def _auto_select_wnaf_window() -> int:
    cfg = _HARDWARE_CFG
    if cfg.backend != "GPU" or not cfg.cupy_available:
        return 0
    sm = cfg.sm_count or 0
    mem_gb = cfg.total_mem_gb or 0.0
    cc_major = 0
    if cfg.compute_capability:
        cc_major = cfg.compute_capability[0]

    if secp_gpu_batch_w8 is not None and mem_gb >= 30.0 and sm >= 80 and cc_major >= 8:
        return 8
    if secp_gpu_batch_w6 is not None and mem_gb >= 12.0 and sm >= 48 and cc_major >= 7:
        return 6
    if secp_gpu_batch_w4 is not None:
        return 4
    return 0


_WNAF_REQUESTED_WINDOW = _normalize_wnaf_window(os.environ.get("VANITY_WNAF_WINDOW", "auto"))
_WNAF_KERNEL_MAP = {
    4: {"kernel": secp_gpu_batch_w4, "warmup": _warmup_window4_table},
    6: {"kernel": secp_gpu_batch_w6, "warmup": _warmup_window6_table},
    8: {"kernel": secp_gpu_batch_w8, "warmup": _warmup_window8_table},
}
_WNAF_TARGET_WINDOW = _WNAF_REQUESTED_WINDOW or _auto_select_wnaf_window()
if _WNAF_REQUESTED_WINDOW == 0 and _WNAF_TARGET_WINDOW:
    logger.info("[WNAF] 自動選擇 Window%d 核心", _WNAF_TARGET_WINDOW)

if _WNAF_TARGET_WINDOW and (
    _WNAF_TARGET_WINDOW not in _WNAF_KERNEL_MAP or _WNAF_KERNEL_MAP[_WNAF_TARGET_WINDOW]["kernel"] is None
):
    fallback_entry = _WNAF_KERNEL_MAP.get(4, {})
    requested_window = _WNAF_TARGET_WINDOW
    if requested_window != 4 and fallback_entry.get("kernel") is not None:
        logger.info("[WNAF] Window%d 核心不可用，改以 Window4 取代", requested_window)
        _WNAF_TARGET_WINDOW = 4
    else:
        if _USE_WNAF_DEFAULT:
            logger.info("[WNAF] 未找到可用的 Window 核心，停用 WNAF 提升")
        _WNAF_TARGET_WINDOW = 0
        _USE_WNAF_DEFAULT = False

_WNAF_FORCE_NO_THRESHOLD = False
if _WNAF_TARGET_WINDOW == 8 and os.environ.get("VANITY_WNAF_KEEP_LIMIT", "0") != "1":
    _WNAF_FORCE_NO_THRESHOLD = True
...
if _USE_WNAF_DEFAULT and _WNAF_TARGET_WINDOW:
    warmup_fn = _WNAF_KERNEL_MAP.get(_WNAF_TARGET_WINDOW, {}).get("warmup")
    kernel_fn = _WNAF_KERNEL_MAP.get(_WNAF_TARGET_WINDOW, {}).get("kernel")
    if warmup_fn is not None:
        try:
            warmup_fn()
            _WNAF_READY = True
            logger.info("[WNAF] 模組初始化完成 Window%d 預熱", _WNAF_TARGET_WINDOW)
...
if _WNAF_FORCE_NO_THRESHOLD:
    _WNAF_BATCH_THRESHOLD = 0
...
logger.info(
    "偵測硬體配置：%s | 預設批次=%s | Streams(base)=%d | Window 門檻=%s | Window 目標=%s",
    _HARDWARE_CFG.summary(),
    _DEFAULT_DYNAMIC_BATCHES,
    _BASE_STREAM_COUNT,
    "disabled" if _WNAF_BATCH_THRESHOLD <= 0 else f"{_WNAF_BATCH_THRESHOLD}",
    f"W{_WNAF_TARGET_WINDOW}" if _WNAF_TARGET_WINDOW else "disabled",
)
...
def _launch_batch(
    ...
    global _WNAF_READY, _WNAF_BROKEN, _WNAF_LAST_USED, _WNAF_SIZE_LOGGED, _WNAF_LAST_REASON, _WNAF_LAST_WINDOW
    ...
    with stream:
        ...
        kernel_entry = _WNAF_KERNEL_MAP.get(_WNAF_TARGET_WINDOW, {})
        kernel_fn = kernel_entry.get("kernel")
        warmup_fn = kernel_entry.get("warmup")
        current_window = _WNAF_TARGET_WINDOW if _WNAF_TARGET_WINDOW else None

        use_kernel = (
            use_wnaf_requested
            and not _WNAF_BROKEN
            and kernel_fn is not None
        )
        if use_kernel and not _WNAF_READY:
            if warmup_fn is not None:
                logger.info("[WNAF] 觸發預熱，準備載入 Window%d 常數表", current_window or 0)
                try:
                    warmup_fn()
                    _WNAF_READY = True
                    logger.info("[WNAF] 常數表初始化完成，開始使用 Window%d 核心", current_window or 0)
                except Exception as exc:  # pragma: no cover
        ...
        if use_kernel:
            if not _WNAF_READY:
                ...
                wnaf_reason = f"window{current_window or 0}"
        else:
            if not use_wnaf_requested:
                wnaf_reason = "adaptive-threshold"
            elif _WNAF_BROKEN:
                wnaf_reason = "broken-flag"
            elif kernel_fn is None:
                wnaf_reason = "kernel-missing"
            else:
                wnaf_reason = "fallback"
        logger.debug(
            "[WNAF] 批次 %d 決策：use=%s reason=%s requested=%s limit=%d ready=%s broken=%s window=%s kernel_ok=%s",
            cur,
            use_kernel,
            wnaf_reason,
            use_wnaf_requested,
        ...
        if use_kernel:
            _WNAF_SIZE_LOGGED = False
            try:
                pub65_gpu = kernel_fn(sk_gpu, out=pub65_gpu)
                _WNAF_LAST_USED = True
                _WNAF_LAST_WINDOW = current_window or 0
                _wnaf_record_success(cur)
            except Exception as exc:
                _WNAF_BROKEN = True
                _WNAF_LAST_WINDOW = 0
                logger.warning(
                    "[WNAF] Window%d 核心執行異常(%s)，改用標準版",
                    current_window or 0,
                    exc,
                    exc_info=True,
                )
...
def generate_tron_addresses_gpu(
    ...
    *,
    ...
    window_tag = f"Window{_WNAF_TARGET_WINDOW}" if _WNAF_TARGET_WINDOW else "Window"
    if use_wnaf_kernel:
        logger.info("[WNAF] 預設啟用 %s 核心，等待預熱完成", window_tag)
    else:
        logger.info("[WNAF] 已停用 Window 核心，改用標準 secp256k1 內核")
    if use_wnaf_kernel:
        if _WNAF_ADAPTIVE_LIMIT > 0:
            logger.info("[WNAF] 初始門檻：批次 > %d 將改用標準內核", _WNAF_ADAPTIVE_LIMIT)
        else:
            logger.info("[WNAF] 未設定門檻，將嘗試所有批次使用 %s", window_tag)
    ...
    if return_stats:
        stats = {
            ...
            "wnaf_ready": _WNAF_READY,
            "wnaf_target_window": _WNAF_TARGET_WINDOW,
            "wnaf_request_window": _WNAF_REQUESTED_WINDOW,
            "wnaf_last_window": _WNAF_LAST_WINDOW,
