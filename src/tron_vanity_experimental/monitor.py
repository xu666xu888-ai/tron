...
class VanitySearchMonitor:
    ...
    def _build_body_panel(self) -> Panel:
        ...
        def _fmt_window(val: Optional[float]) -> str:
            try:
        ...
        window_request = extras.get("wnaf_request_window")
        window_target = extras.get("wnaf_target_window")
        window_last = extras.get("wnaf_last_window")
        window_used_any = extras.get("wnaf_used")
        window_used_last = extras.get("wnaf_last_used")
        kernel_enabled = extras.get("wnaf_kernel_enabled")
        kernel_broken = extras.get("wnaf_broken")
        kernel_ready = extras.get("wnaf_ready")

        if window_request is not None:
            request_label = _fmt_window(window_request)
            target_label = _fmt_window(window_target)
            label = request_label if request_label == target_label else f"{request_label} → {target_label}"
            pipeline_table.add_row("Window 設定", label)

        if kernel_enabled is not None or kernel_broken is not None:
            if kernel_broken:
                kernel_state = "異常"
            elif kernel_enabled:
                tag = _fmt_window(window_target)
                kernel_state = f"啟用 · {tag}" if kernel_ready or kernel_ready is None else f"啟用(預熱) · {tag}"
            else:
                kernel_state = "停用"
            pipeline_table.add_row("Window 核心", kernel_state)

        if window_used_any is not None:
            pipeline_table.add_row("Window 累積", "啟用" if window_used_any else "停用")

        if window_used_last is not None:
            last_label = _fmt_window(window_last)
            pipeline_table.add_row(
                "Window 本批",
                f"啟用 · {last_label}" if window_used_last and last_label != "停用" else "停用",
            )
        ...
        if extras.get("wnaf_limit") is not None:
            ...
            if limit_val and limit_val > 0:
                ...
                if base_val and base_val > 0 and limit_val > base_val:
                    ...
                    label += " ↓"
                pipeline_table.add_row("Window 門檻", label)
            else:
                pipeline_table.add_row("Window 門檻", "無限制")
        ...
        if extras.get("wnaf_skip_streak") is not None:
            pipeline_table.add_row("Window 跳過", f"{int(extras['wnaf_skip_streak'])} 次")
        if extras.get("wnaf_success_streak") is not None:
            pipeline_table.add_row("Window 連勝", f"{int(extras['wnaf_success_streak'])} 次")
        if extras.get("wnaf_promotions") is not None:
            pipeline_table.add_row("Window 調整", f"{int(extras['wnaf_promotions'])} 次")
        if extras.get("wnaf_last_reason"):
            reason = str(extras["wnaf_last_reason"])
            if len(reason) > 36:
                reason = reason[:33] + "..."
            pipeline_table.add_row("Window 決策", reason)
